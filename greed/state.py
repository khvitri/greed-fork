...
from greed.state_plugins import (
    SimStatePlugin,
    SimStateSolver,
    SimStateGlobals,
    SimStateInspect,
...
class SymbolicEVMState:
    ...
    def __init__(
        ...
        self.ctx = dict()
        self.callstack = list()
        self.returndata = {"size": BVV(
            0, 256), "instruction_count": BVV(0, 256)}
        ...
        self.balance = BV_Add(
            self.start_balance, ctx_or_symbolic(
                "CALLVALUE", self.ctx, self.xid)
        )
        ...
        self.min_timestamp = (
            datetime.datetime(2022, 1, 1) - datetime.datetime(1970, 1, 1)
        ).total_seconds()
        self.max_timestamp = (
        ...
        if not partial_concrete_storage:
            ...
            self.storage = LambdaMemory(
                tag=f"STORAGE_{self.xid}", value_sort=BVSort(256), state=self
            )
        else:
            log.debug("Using PartialConcreteStorage")
            self.storage = PartialConcreteStorage(
                tag=f"PCONCR_STORAGE_{self.xid}", value_sort=BVSort(256), state=self
            )
    ...
    def set_init_ctx(self, init_ctx=None):
        ...
        if "CALLDATASIZE" in init_ctx:
            ...
            self.add_constraint(
                BV_ULE(self.calldatasize, BVV(self.MAX_CALLDATA_SIZE, 256))
            )
        ...
        if "CALLDATA" in init_ctx:
            ...
            assert isinstance(init_ctx["CALLDATA"], str), (
                "Wrong type for CALLDATA initial context"
            )

            # Parse the CALLDATA (divide the input string byte by byte)
            calldata_raw = init_ctx["CALLDATA"].replace("0x", "")
            calldata_bytes = [
                calldata_raw[i: i + 2] for i in range(0, len(calldata_raw), 2)
            ]

            if "CALLDATASIZE" in init_ctx:
                self.calldata = LambdaMemory(
                    tag=f"CALLDATA_{self.xid}", value_sort=BVSort(8), state=self
                )

                assert init_ctx["CALLDATASIZE"] >= len(calldata_bytes), (
                    "CALLDATASIZE is smaller than len(CALLDATA)"
                )
            else:
                self.calldata = LambdaMemory(
                    tag=f"CALLDATA_{self.xid}", value_sort=BVSort(8), state=self
                ...
                self.add_constraint(
                    BV_UGE(self.calldatasize, BVV(len(calldata_bytes), 256))
                )
            ...
            for index, cb in enumerate(calldata_bytes):
                if cb == "SS":
                    ...
                    self.calldata[BVV(index, 256)] = BVS(
                        f"CALLDATA_BYTE_{index}", 8)
                else:
        ...
        if "CALLER" in init_ctx:
            assert isinstance(init_ctx["CALLER"], str), (
                "Wrong type for CALLER initial context"
            )
            self.ctx["CALLER"] = BVV(int(init_ctx["CALLER"], 16), 256)

        if "ORIGIN" in init_ctx:
            assert isinstance(init_ctx["ORIGIN"], str), (
                "Wrong type for ORIGIN initial context"
            )
            self.ctx["ORIGIN"] = BVV(int(init_ctx["ORIGIN"], 16), 256)

        if "BALANCE" in init_ctx:
            assert isinstance(init_ctx["BALANCE"], int), (
                "Wrong type for BALANCE initial context"
            )
            self.add_constraint(
                Equal(self.start_balance, BVV(init_ctx["BALANCE"], 256))
            )

        if "ADDRESS" in init_ctx:
            assert isinstance(init_ctx["ADDRESS"], str), (
                "Wrong type for ADDRESS initial context"
            )
            self.ctx["ADDRESS"] = BVV(int(init_ctx["ADDRESS"], 16), 256)

        if "NUMBER" in init_ctx:
            assert isinstance(init_ctx["NUMBER"], int), (
                "Wrong type for NUMBER initial context"
            )
            self.ctx["NUMBER"] = BVV(init_ctx["NUMBER"], 256)

        if "DIFFICULTY" in init_ctx:
            assert isinstance(init_ctx["DIFFICULTY"], int), (
                "Wrong type for DIFFICULTY initial context"
            )
            self.ctx["DIFFICULTY"] = BVV(init_ctx["DIFFICULTY"], 256)

        if "TIMESTAMP" in init_ctx:
            assert isinstance(init_ctx["TIMESTAMP"], int), (
                "Wrong type for TIMESTAMP initial context"
            )
            self.ctx["TIMESTAMP"] = BVV(init_ctx["TIMESTAMP"], 256)

        if "CALLVALUE" in init_ctx:
            assert isinstance(init_ctx["CALLVALUE"], int), (
                "Wrong type for CALLVALUE initial context"
            )
    ...
    def get_fallthrough_pc(self):
        ...
        if len(curr_bb.succ) == 0:
            ...
            raise VMNoSuccessors(
                f"Block {curr_bb} does not have a fallthrough edge.")
        else:
    ...
    def get_non_fallthrough_pc(self, destination_val):
        ...
        if not is_concrete(destination_val):
            raise VMSymbolicError(
                f"Symbolic jump destination currently not supported. "
                f"({destination_val=})"
            )
        else:
        ...
        candidate_destination_vals = self.project.tac_parser.statement_to_blocks_map[
            destination_val
        ] + [destination_val]
        candidate_bbs = [
            bb
            for bb in curr_bb.succ
            if (bb.id in candidate_destination_vals)
            or ("0x" + bb.id.split("0x")[1] in candidate_destination_vals)
        ]

        if len(candidate_bbs) == 0:
            raise VMSymbolicError(
                f"Unable to find jump destination. "
                f"({candidate_destination_vals=}, {curr_bb.succ=})"
            )
        elif len(candidate_bbs) > 1:
            raise VMSymbolicError(
                f"Multiple jump destinations. "
                f"({candidate_destination_vals=}, {curr_bb.succ=})"
            )
    ...
    def copy(self):
        ...
        new_state = SymbolicEVMState(
            self.xid, project=self.project, partial_init=True)
        ...
        new_state.sha_observed = [
            sha.copy(new_state=new_state) for sha in self.sha_observed
        ]
